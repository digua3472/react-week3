...
function App() {
  ...
  const getProducts = async () => {
    try {
      const res = await axios.get(`${URL}/v2/api/${PATH}/admin/products`);
  ...
  const createProduct = async () => {
    try {
      await axios.post(`${URL}/v2/api/${PATH}/admin/product`, {
        data: {
          ...tempProduct,
  ...
  const updateProduct = async () => {
    try {
      await axios.put(`${URL}/v2/api/${PATH}/admin/product/${tempProduct.id}`, {
        data: {
          ...tempProduct,
          origin_price: Number(tempProduct.origin_price),
          price: Number(tempProduct.price),
  ...
  const deleteProduct = async () => {
    try {
      await axios.delete(`${URL}/v2/api/${PATH}/admin/product/${tempProduct.id}`)
    } catch (error) {
      alert('刪除產品失敗');
    }
  }
